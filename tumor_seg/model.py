...
class SelectiveUNet(nn.Module):
    def __init__(self, input_type = 'RGB', n_cls = 2):
        super(SelectiveUNet, self).__init__()
    ...
    def forward(self, x):
        ...
        output = self.sigmoid(self.conv1x1(dec1_1)) # (N, C, H, W)

        select = self.sigmoid(self.conv_select(dec1_1)) # (N, 2, H, W)
        aux = self.sigmoid(self.conv_aux(dec1_1)) # (N, C, H, W)
